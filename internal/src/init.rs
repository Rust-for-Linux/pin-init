...
use syn::{
    ...
    spanned::Spanned,
    token, Block, Error, Expr, ExprCall, ExprPath, Path, Result, Token, Type,
};
...
impl FieldInitializer {
    fn ident(&self) -> Option<&Ident> {
        match self {
            FieldInitializer::Value { ident, .. } | FieldInitializer::Init { ident, .. } => {
                Some(ident)
            }
            FieldInitializer::Code { .. } => None,
        }
    }
}
...
fn init_fields(
    ...
    for field in fields {
        let init = match field {
            ...
            FieldInitializer::Code { block: value, .. } => quote!(#value),
        };
        res.extend(init);
            ...
            fields: {
                ...
                loop {
                    ...
                    if content.is_empty() || lookahead.peek(Token![..]) {
                        break;
                    } else if lookahead.peek(syn::Ident) || lookahead.peek(Token![_]) {
